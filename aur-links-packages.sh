...
do
  ...
  aur_sources_link_curl=$(curl -s $aur_link | grep -A50 '<div id="pkgfiles" class="listing">' | grep '<a href' | sed '/dcelasun/d')
#  aur_curl_link=$(curl -s $aur_link | grep '<a href="https://github.com/*' | sed '1d')
#  echo $aur_curl_link
  echo "Исходников в пакете ${aur_curl_count}"
  echo -e $aur_sources_link_curl\n
