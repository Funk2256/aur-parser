...
do
  ...
  aur_sources_link_curl_cgit=$(curl -s https://aur.archlinux.org/packages/visual-studio-code-bin | grep -A50 '<div id="pkgfiles" class="listing">' | grep '<a href' | sed '/dcelasun/d' | tr -d ' ' | grep -Eo 'http.*"' | tr -d '"')
...
#  aur_sources_link_curl=$(curl -s $aur_link | grep -A50 '<div id="pkgfiles" class="listing">' | grep '<a href' | sed '/dcelasun/d' | tr -d ' ' | grep -Eo 'http.*"' | tr -d '"')
  ...
  echo $aur_sources_link_curl_cgit
  echo $aur_sources_link_curl_cgit >> links_packages.txt
